"ada.modification" <-
  function(x,y,test.x,test.y=NULL,loss=c("exponential","logistic"),
           type=c("discrete","real","gentle"),C_t=10, ctype=1, lc=5, ltype=1,
           type2=c("truncated", "rescale","rescale_truncated","ada","shrinkage"),iter=50, nu=1, bag.frac=0.5,
  ...
  result =ada.machine.modification(x,ny,test.x,test.y,iter,nu,bag.frac,lossObj,C_t,ctype,lc,ltype,type2="truncated",oldObj=NULL,
                                    na.action=na.action,...)}
  if(type2=="rescale")
  {lossObj=list(coefs=coefs,wfun=wfun,predict.type=predict.type,method=method,type=type,loss=loss,shift=bag.shift)
  result =ada.machine.modification(x,ny,test.x,test.y,iter,nu,bag.frac,lossObj,C_t,ctype,lc,ltype,type2="rescale",oldObj=NULL,
                                    na.action=na.action,...)}
  
  if(type2=="rescale_truncated")
  {lossObj=list(coefs=coefs,wfun=wfun,predict.type=predict.type,method=method,type=type,loss=loss,shift=bag.shift)
  result =ada.machine.modification(x,ny,test.x,test.y,iter,nu,bag.frac,lossObj,C_t,ctype,lc,ltype,type2="rescale_truncated",oldObj=NULL,
                                    na.action=na.action,...)}
  
  if(type2=="ada")
  {lossObj=list(coefs=coefs,wfun=wfun,predict.type=predict.type,method=method,type=type,loss=loss,shift=bag.shift)
  result =ada.machine.modification(x,ny,test.x,test.y,iter,nu,bag.frac,lossObj,C_t,ctype,lc,ltype,type2="ada",oldObj=NULL,
                                   na.action=na.action,...)}
  if(type2=="shrinkage")
  {lossObj=list(coefs=coefs,wfun=wfun,predict.type=predict.type,method=method,type=type,loss=loss,shift=bag.shift)
  result =ada.machine.modification(x,ny,test.x,test.y,iter,nu,bag.frac,lossObj,C_t,ctype,lc,ltype,type2="shrinkage",oldObj=NULL,
                                   na.action=na.action,...)}
  ...
  obj=structure(list(model=result,fit=g,call=cl,confusion=tab,iter=iter,
    actual=as.vector(y),nu=nu,dim=dim(x),names=nm,bag.frac=bag.frac,na.action=na.action,type2=type2))
  obj
}
