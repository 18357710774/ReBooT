"cvada.modification" <-
  function(x,y,test.x,test.y=NULL,loss=c("exponential","logistic"),
           ...
           type2=c("truncated", "rescale","rescale_truncated","ada","shrinkage"), iter=50, nu=1,
    ...
    result =cvada.machine.modification(x,ny,test.x,test.y,iter,nu,lossObj,C_t,ctype,lc,ltype, type2="truncated",oldObj=NULL,
                                       na.action=na.action,...)}
    
    if(type2=="rescale")
    {lossObj=list(coefs=coefs,wfun=wfun,predict.type=predict.type,method=method,type=type,loss=loss,shift=bag.shift)
    result =cvada.machine.modification(x,ny,test.x,test.y,iter,nu,lossObj,C_t,ctype,lc,ltype, type2="rescale",oldObj=NULL,
                                       na.action=na.action,...)}
    
    if(type2=="rescale_truncated")
    {lossObj=list(coefs=coefs,wfun=wfun,predict.type=predict.type,method=method,type=type,loss=loss,shift=bag.shift)
    result =cvada.machine.modification(x,ny,test.x,test.y,iter,nu,lossObj,C_t,ctype,lc,ltype, type2="rescale_truncated",oldObj=NULL,
                                       na.action=na.action,...)}
    
    if(type2=="ada")
    {lossObj=list(coefs=coefs,wfun=wfun,predict.type=predict.type,method=method,type=type,loss=loss,shift=bag.shift)
    result =cvada.machine.modification(x,ny,test.x,test.y,iter,nu,lossObj,C_t,ctype,lc,ltype, type2="ada",oldObj=NULL,
                                     na.action=na.action,...)}
    
    if(type2=="shrinkage")
    {lossObj=list(coefs=coefs,wfun=wfun,predict.type=predict.type,method=method,type=type,loss=loss,shift=bag.shift)
    result =cvada.machine.modification(x,ny,test.x,test.y,iter,nu,lossObj,C_t,ctype,lc,ltype,type2="shrinkage",oldObj=NULL,
                                     na.action=na.action,...)}
    
    g=as.factor(lev[as.numeric(as.factor(sign(result$Fval[[1]])))])
    ...
    obj=structure(list(model=result,fit=g,call=cl,confusion=tab,iter=iter,
                       actual=as.vector(y),nu=nu,dim=dim(x),names=nm,na.action=na.action,type2=type2))
