CVboosting_truncated <- function(train_x, train_y, iter_tmp=500,
  ...
  ## Main function for cross-validation of RTboosting algorithm
  ...
  if (parallel == TRUE){
    ...
    Errlist = foreach(num = seq(nfolds), .packages =c('rpart','stringr')) %dopar% {
      ...
      for (m in 1:num_c){
        ...
        g_truncated<- cvada.modification(x=fold_train_x, y=fold_train_y, test.x=fold_test_x, test.y=fold_test_y, loss=loss, 
                                         type2="truncated", type="real", iter=iter_tmp, C_t=c_tmp, nu=1, control=control)
        
        test_err[,m] = g_truncated$model$err[,3]
        train_err[,m] = g_truncated$model$err[,1]
