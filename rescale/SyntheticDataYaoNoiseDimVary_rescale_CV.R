## Main function for the performance comparison of RBoosting on the high-cross data with changing number of noise dimensions
...
source(str_c(pathtmp, "rescale\\CVboosting_rescale.R"))
source(str_c(pathtmp, "rescale\\boosting_rescale.R"))

# parameter setting
iterMax = 10000
c_seq = GeometricSeries(0.1, 100, 21)
nfolds = 5
...
for (ii in 1:length(noise_feature_Cross)){
  ...
  file1 = str_c(pathtmp,"rescale\\results\\R_", dataName, "_CVSel_results_iterMax",
  ...
  rescale_CVresults <- list()
  rescale_results_IterOpt <- list()
  ...
  while(tt<=t_num) # loops
  {
    ...
    rescale_CVresults[[tt]] <- CVboosting_rescale(train_x, train_y, iter_tmp = iterMax, c_seq=c_seq, loss=loss, ctype=ctype,
                                                  control=control, nfolds = nfolds, parallel = parallel, pathtmp = pathtmp)
    
  
    # select the optimal values of C and the number of iterations
    c = rescale_CVresults[[tt]]$ParaCVSelIterAll$c
    Iter = rescale_CVresults[[tt]]$ParaCVSelIterAll$IterOpt
    c_IterOpt[tt] = c[1]
    IterOpt[tt] = Iter[1]
  
  
    rescale_results_IterOpt[[tt]] <- boosting_rescale(train_x, train_y, test_x, test_y, iter_tmp=iterMax,
                                                      c_seq=c_IterOpt[tt], loss=loss, ctype=ctype, control = control)
                                                      
    
    test_err_IterOptAll[,tt] = rescale_results_IterOpt[[tt]]$test_err
    train_err_IterOptAll[,tt] = rescale_results_IterOpt[[tt]]$train_err
    alpha_IterOptAll[,tt] = rescale_results_IterOpt[[tt]]$alpha
    l1norm_IterOptAll[,tt] = rescale_results_IterOpt[[tt]]$l1norm
    
    test_err_IterOpt[tt] = rescale_results_IterOpt[[tt]]$test_err[IterOpt[tt]]
    train_err_IterOpt[tt] = rescale_results_IterOpt[[tt]]$train_err[IterOpt[tt]]
    alpha_IterOpt[[tt]] = rescale_results_IterOpt[[tt]]$alpha
    l1norm_IterOpt[tt] = rescale_results_IterOpt[[tt]]$l1norm[IterOpt[tt]]
  ...
  R_result_list = list(CVresults = rescale_CVresults, results = results)
  
  save(R_result_list,file=file1)
}
