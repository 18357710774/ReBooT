...
for (ii in 1:length(noise_feature_Cross)){
  ...
  savefile = str_c(savepath,"SynDataTrN200NoiDim", as.character(noise_feature), ".Rdata")
  ...
  while(tt<=t_num) # loops
  {
    ...
    p_size_train <- N/2
    n_size_train <- N/2
    data_train <- sinusoid_lowcross_data(p_size_train,n_size_train, noise_feature)
    ...
    p_size_test <- 2000
    n_size_test <- 2000
    data_test <- sinusoid_lowcross_data(p_size_test,n_size_test,noise_feature)
