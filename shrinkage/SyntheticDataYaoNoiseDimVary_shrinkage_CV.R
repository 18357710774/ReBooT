## Main function for the performance comparison of RSboosting on the high-cross data with changing number of noise dimensions
...
source(str_c(pathtmp, "shrinkage\\CVboosting_shrinkage.R"))
source(str_c(pathtmp, "shrinkage\\boosting_shrinkage.R"))

# parameter setting
iterMax = 10000
nu_seq = GeometricSeries(0.01, 1, 21)
nfolds = 5
max_depth = 1
loss = 'l'
parallel = TRUE
...
for (ii in 1:length(noise_feature_Cross)){
  ...
  file1 = str_c(pathtmp,"shrinkage\\results\\S_", dataName, "_CVSel_results_iterMax",
  ...
  while(tt<=t_num) # loops
  {
    ...
    shrinkage_CVresults[[tt]] <-  CVboosting_shrinkage(train_x, train_y, iter_tmp = iterMax, nu_seq=nu_seq, loss=loss,
                                                       control=control, nfolds=nfolds, parallel=parallel, pathtmp = pathtmp)
    
    # select the optimal values of nu and the number of iterations
    nu = shrinkage_CVresults[[tt]]$ParaCVSelIterAll$nu
    Iter = shrinkage_CVresults[[tt]]$ParaCVSelIterAll$IterOpt
    nu_IterOpt[tt] = nu[1]
    IterOpt[tt] = Iter[1]
    
    
    shrinkage_results_IterOpt[[tt]] <- boosting_shrinkage(train_x, train_y, test_x, test_y, iter_tmp=iterMax, 
                                                          control = control, nu_seq=nu_IterOpt[tt], loss=loss)
    
    
    test_err_IterOptAll[,tt] = shrinkage_results_IterOpt[[tt]]$test_err
    train_err_IterOptAll[,tt] = shrinkage_results_IterOpt[[tt]]$train_err
    alpha_IterOptAll[,tt] = shrinkage_results_IterOpt[[tt]]$alpha
    l1norm_IterOptAll[,tt] = shrinkage_results_IterOpt[[tt]]$l1norm
    
    test_err_IterOpt[tt] = shrinkage_results_IterOpt[[tt]]$test_err[IterOpt[tt]]
    train_err_IterOpt[tt] = shrinkage_results_IterOpt[[tt]]$train_err[IterOpt[tt]]
    alpha_IterOpt[[tt]] = shrinkage_results_IterOpt[[tt]]$alpha
    l1norm_IterOpt[tt] = shrinkage_results_IterOpt[[tt]]$l1norm[IterOpt[tt]]
  ...
  results = list(test_err_IterOpt = test_err_IterOpt, train_err_IterOpt = train_err_IterOpt, alpha_IterOpt = alpha_IterOpt, 
                 l1norm_IterOpt = l1norm_IterOpt, nu_IterOpt= nu_IterOpt, IterOpt = IterOpt,
                 ...
                 MeanStd = list(teMeanStd_IterOpt = c(mean(test_err_IterOpt), sd(test_err_IterOpt)), 
                                ...
                                nuMeanStd_IterOpt = c(mean(nu_IterOpt),sd(nu_IterOpt)),
  ...
  S_result_list = list(CVresults = shrinkage_CVresults, results = results)
  
  save(S_result_list,file=file1)
}