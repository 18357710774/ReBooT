## Main function for the parameter analysis of RSboosting on the low-cross data with 12 dimensions of noise
...
nu_seq <- c(0.01,seq(0.05,1,length.out=20))
...
savefile = str_c(pathtmp,"shrinkage\\results\\S_", dataName, "_ParaAnalysis_iterMax",
...
shrinkage_result <- as.list(seq(t_num))

if (parallel == TRUE){
    no_cores <- detectCores()
    cl <- makeCluster(no_cores-2)
    registerDoParallel(cl)
    shrinkage_result =foreach(num = seq(t_num), .packages =c('rpart','stringr')) %dopar% {
    source(str_c(pathtmp, "ada.modification.R"))
    source(str_c(pathtmp, "ada.machine.modification.R"))
    source(str_c(pathtmp, "shrinkage\\boosting_shrinkage.R"))
    source(str_c(pathtmp, "l1normComputation.R"))
    ...
    obj = boosting_shrinkage(train_x, train_y, test_x, test_y, iter_tmp=iterMax, 
                             control = control, nu_seq=nu_seq, loss=loss)
  }
  stopCluster(cl) 
}else
  {
  while(tt<=t_num) # loops
  {
    ...
    shrinkage_result[[tt]] <- boosting_shrinkage(train_x, train_y, test_x, test_y, iter_tmp=iterMax, 
                                                 control = control, nu_seq=nu_seq, loss=loss)
    print(tt)
    tt <- tt+1
  }
}
save(shrinkage_result,file=savefile)
