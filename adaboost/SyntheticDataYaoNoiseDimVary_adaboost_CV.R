## Main function for the performance comparison of AdaBoost on the high-cross data with changing number of noise dimensions
...
source(str_c(pathtmp, "adaboost\\CVboosting_ada_exp.R"))
...
for (ii in 1:length(noise_feature_Cross)){
  ...
  file1 = str_c(pathtmp,"adaboost\\results\\Aexp_", dataName, "_CVSel_results_iterMax",
  ...
  while(tt<=t_num) # loops
  {
    ...
    ada_CVresults[[tt]] <- CVboosting_ada_exp(train_x, train_y, iter_tmp = iterMax, control=control, loss=loss,
    ...
    ada_results_IterOpt[[tt]] <- boosting_ada_exp(train_x, train_y, test_x, test_y, iter_tmp=iterMax, 
  ...
  Aexp_result_list = list(CVresults = ada_CVresults, results = results)
  
  save(Aexp_result_list,file=file1)
}
